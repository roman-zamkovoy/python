...
def lose(a):
    for i in range(3):
        ...
        if (a[i][0] == "x" and a[i][1] == "_" and a[i][2] == "x"):  # СТРОКа
            return [i,1]
        if (a[i][0] == "_" and a[i][1] == "x" and a[i][2] == "x"):  # СТРОКа
            return [i,0]
    # ДЗ - сделать такую же проверку по столбцам и ДИАГОНАЛЯМ
    for i in range(3):
        if (a[0][i] == "x" and a[1][i] == "x" and a[2][i] == "_"):  # СТРОКа
            return [2, i]
        if (a[0][i] == "x" and a[1][i] == "_" and a[2][i] == "x"):  # СТРОКа
            return [1, i]
        if (a[0][i] == "_" and a[1][i] == "x" and a[2][i] == "x"):  # СТРОКа
            return [0, i]
    if (a[0][0] == "x" and a[1][1] == "x" and a[2][2] == "_"):  # СТРОКа
        return [2, 2]
    if (a[0][0] == "x" and a[1][1] == "_" and a[2][2] == "x"):  # СТРОКа
        return [1,1]
    if (a[ 0][0] == "_" and a[1][1] == "x" and a[2][2] == "x"):  # СТРОКа
        return [0,0]
    return []
